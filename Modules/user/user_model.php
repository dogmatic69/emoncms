...
class User {
    ...
    public function logout() {
        ...
        $_SESSION['userid'] = $_SESSION['read'] = $_SESSION['write'] = $_SESSION['admin'] = 0;
    ...
    public function change_password($userid, $old, $new) {
        ...
        if (strlen($old) < 4 || strlen($old) > 30 || strlen($new) < 4 || strlen($new) > 30) {
            return array('success' => false, 'message' => _('Password length error'));
        }
    ...
    public function change_username($userid, $username) {
        if (isset($_SESSION['cookielogin']) && $_SESSION['cookielogin']==true) {
            return array('success'=>false, 'message'=>_("As your using a cookie based remember me login, please logout and log back in to change username"));
        }
    ...
    public function change_email($userid, $email) {
        if (isset($_SESSION['cookielogin']) && $_SESSION['cookielogin']==true) {
            return array('success'=>false, 'message'=>_("As your using a cookie based remember me login, please logout and log back in to change email"));
        }
